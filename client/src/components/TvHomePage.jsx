...
import { fetchTvStatus } from '../api';

export default function TvHomePage({ onQueue, onLibrary, onBack }) {
  const [stats, setStats] = useState(null);

  useEffect(() => {
    fetchTvStatus().then(setStats).catch(() => {});
  }, []);

  return (
    <div style={s.page}>
      <header style={s.header}>
        <h1 style={s.title}>TV Show Artwork</h1>
        <p style={s.sub}>Select and download artwork for your TV library</p>
      </header>
      ...
      <div className="home-cards">
        ...
        <Card
          ...
          ctaColor="#0f766e"
          onClick={onLibrary}
          badge={stats ? `${stats.total} shows` : null}
          badgeColor="#555"
        />
      </div>

      <div style={s.backRow}>
        <button onClick={onBack} style={s.backBtn}>← Back to Home</button>
      </div>
    </div>
  );
}
...
const s = {
  ...
  backBtn:   { background: '#1e1e38', color: '#8080a0', padding: '7px 16px', fontSize: 13 },
};
